  ...
  $.fn.birthdaypicker = function( options ) {
    ...
    return this.each(function() {
      ...
      var $fieldset = $("<fieldset class='birthday-picker'></fieldset>"),
          $year = $("<select class='birth-year form-control mr-1 custom-select validate[required]' name='birth_year'></select>"),
          $month = $("<select class='birth-month form-control mr-1 custom-select validate[required]' name='birth_month'></select>"),
          $day = $("<select class='birth-day form-control mr-1 custom-select validate[required]' name='birth_day'></select>");
      ...
      if (settings["placeholder"]) {
        $("<option value=''>年</option>").appendTo($year);
        $("<option value=''>月</option>").appendTo($month);
        $("<option value=''>日</option>").appendTo($day);
      }
      ...
      for (var j=0; j<12; j++) {
        if(j<9) {
          $("<option></option>").attr("value", "0"+(j+1)).text(months[settings["monthFormat"]][j]).appendTo($month); 
        } else {
      ...
      for (var k=1; k<32; k++) {
        if(k<10) {
          $("<option></option>").attr("value", "0"+k).text("0"+k).appendTo($day);
        } else {
